...
fn parse_elf_bytes_inner(
    ...
    if data.len() < 4 || &data[0..4] != b"\x7fELF" {
        return Err(anyhow!("not ELF: {}", path));
    }
    let is64 = data[4] == 2;
    ...
    let e_entry = if is64 {
        ...
        u32::from_le_bytes(data[24..28].try_into()?) as u64
    };
    ...
    if !no_notes {
        for ph in &elf.program_headers {
            ...
            while cur + 12 <= end && cur + 12 <= data.len() {
                let n_namesz =
                    u32::from_le_bytes(data[cur..cur + 4].try_into().unwrap_or([0; 4])) as usize;
                let n_descsz =
                    u32::from_le_bytes(data[cur + 4..cur + 8].try_into().unwrap_or([0; 4]))
                        as usize;
                let n_type =
                    u32::from_le_bytes(data[cur + 8..cur + 12].try_into().unwrap_or([0; 4]));
                cur += 12;
                ...
                let desc = if n_descsz > 0 && cur + n_descsz <= data.len() {
                    Some(data[cur..cur + n_descsz].to_vec())
                } else {
                    None
                };
                cur = (cur + n_descsz + 3) & !3;
                notes.push(Note {
                    typ: n_type.to_string(),
                    name,
                    desc,
                });
            }
    ...
    let syms_iter = if !elf.dynsyms.is_empty() {
        &elf.dynsyms
    } else {
        &elf.syms
    };
    let strtab = if !elf.dynsyms.is_empty() {
        &elf.dynstrtab
    } else {
        &elf.strtab
    };
    ...
    for sym in syms_iter.iter() {
        ...
        let exported = if defined == 1 && (bind == 1 || bind == 2) {
            1
        } else {
            0
        };
...
pub fn parse_elf(path: &str, no_sections: bool, no_notes: bool) -> Result<ElfInfo> {
    let file = match File::open(path) {
        Ok(f) => f,
        Err(e) => return Err(anyhow!("open {}: {}", path, e)),
    };
...
pub fn needed_for_path(path: &str) -> Vec<String> {
    meta_for_path(path).needed
}
pub fn soname_for_path(path: &str) -> Option<String> {
    meta_for_path(path).soname
}
...
pub fn soname_for_path_cached(
    path: &Path,
    cache: &mut FxHashMap<PathBuf, ElfMeta>,
) -> Option<String> {
    meta_for_path_cached(path, cache).soname
}
pub fn runpath_for_path_cached(
    path: &Path,
    cache: &mut FxHashMap<PathBuf, ElfMeta>,
