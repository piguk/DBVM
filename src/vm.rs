...
fn cache_dir() -> PathBuf {
    if let Ok(p) = std::env::var("SELF_VM_CACHE") { return PathBuf::from(p); }
    if let Ok(x) = std::env::var("XDG_CACHE_HOME") { return PathBuf::from(x).join("self-vm"); }
    if let Ok(h) = std::env::var("HOME") { return PathBuf::from(h).join(".cache/self-vm"); }
...
fn now_secs() -> i64 { std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs() as i64 }
...
fn compress_bytes(data: &[u8]) -> Option<Vec<u8>> {
    ...
    if enc.write_all(data).is_err() { return None; }
    if let Ok(c) = enc.finish() {
        if c.len() + 128 < data.len() { return Some(c); }
    }
    None
}
...
fn is_gzipped(data: &[u8]) -> bool { data.len()>=2 && data[0]==0x1f && data[1]==0x8b }
...
fn fx_hash_u64(data: &[u8]) -> String { format!("{:x}", fx_hash(data)) }
...
pub fn vm_stat(conn: &Connection, vm_path: &str) -> Result<(String,String,i64,i64,i64,String)> {
    let p = normalize_vm_path(vm_path);
    let row: Option<(String,String,i64,i64,i64,Option<String>,Option<Vec<u8>>,Option<i64>)> = conn.query_row("SELECT path,kind,mode,size,mtime,link_target,content,compressed FROM vm_fs WHERE path=?1", params![p], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?, r.get(5)?, r.get(6)?, r.get(7)?))).optional()?;
    let (path,kind,mode,size,mtime,link,content,compressed) = match row { Some(x)=>x, None=> return Err(anyhow!("not found: {}", p)) };
    if kind == "symlink" {
    ...
    let hash = if let Some(c)=content {
        ...
        let raw = if is_c { decompress_bytes(&c).unwrap_or(c.clone()) } else { c.clone() };
        let raw2 = if !is_c && is_gzipped(&c) { decompress_bytes(&c).unwrap_or(raw) } else { raw };
...
pub fn vm_resolve(conn: &Connection, vm_path: &str) -> Result<(String, String, Option<Vec<u8>>, Option<String>, i64)> {
    ...
    for _ in 0..40 {
        ...
        let row: Option<(String, Option<Vec<u8>>, Option<String>, i64, Option<i64>)> = conn.query_row("SELECT kind, content, link_target, mode, compressed FROM vm_fs WHERE path=?1", params![cur], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?))).optional()?;
        let (kind, content_opt, link, mode, compressed) = match row { Some(x)=>x, None=> return Err(anyhow!("not found in VM: {}", cur)) };
        if kind != "symlink" {
            let content = match content_opt {
                Some(b) => {
                    let is_c = compressed.unwrap_or(0) != 0;
                    if is_c { Some(decompress_bytes(&b).unwrap_or(b)) }
                    else if is_gzipped(&b) { match decompress_bytes(&b){ Ok(v)=>Some(v), Err(_)=>Some(b)} }
                    else { Some(b) }
                }, None=>None,
            };
...
pub fn vm_import_tar(conn: &Connection, tar_path: &str, strip_prefix: &str) -> Result<usize> {
    ...
    let reader: Box<dyn Read> = if is_gz { Box::new(flate2::read::GzDecoder::new(file)) } else { Box::new(file) };
...
pub fn vm_materialize_tree(conn: &Connection, dest: &Path) -> Result<usize> {
    ...
    let mut st = conn.prepare("SELECT path, content, mode, hash, compressed FROM vm_fs WHERE kind='file'")?;
    let mut n=0usize;
    for r in st.query_map([], |r| Ok((r.get::<_,String>(0)?, r.get::<_,Option<Vec<u8>>>(1)?, r.get::<_,i64>(2)?, r.get::<_,Option<String>>(3)?, r.get::<_,Option<i64>>(4)?)))? {
        let (p,content,mode,hash_opt,compressed)=r?;
        if let Some(blob)=content {
            let data: Vec<u8> = {
                let is_c = compressed.unwrap_or(0)!=0;
                if is_c { decompress_bytes(&blob).unwrap_or(blob.clone()) }
                else if is_gzipped(&blob) { decompress_bytes(&blob).unwrap_or(blob.clone()) }
                else { blob.clone() }
            };
            ...
            let need_write = if host.exists() {
                if let Ok(md)=std::fs::metadata(&host) {
                    if md.len() as i64 == data.len() as i64 {
                        if let Ok(existing)=std::fs::read(&host) { fx_hash_u64(&existing) != hash } else { true }
                    } else { true }
                } else { true }
            } else { true };
            if !need_write { n+=1; continue; }
            if let Some(par)=host.parent(){ std::fs::create_dir_all(par)?; }
            let cache_p = cache_path_for_hash(&hash);
...
pub fn vm_sync_from_host(conn: &Connection, host_root: &Path) -> Result<(usize,usize,usize)> {
    ...
    let mut db_map: HashMap<String, (String, Option<Vec<u8>>, Option<String>, i64)> = HashMap::new();
    {
        let mut st = conn.prepare("SELECT path, kind, content, link_target, mode, compressed FROM vm_fs")?;
        for r in st.query_map([], |r| Ok((r.get::<_,String>(0)?, r.get::<_,String>(1)?, r.get::<_,Option<Vec<u8>>>(2)?, r.get::<_,Option<String>>(3)?, r.get::<_,i64>(4)?, r.get::<_,Option<i64>>(5)?)))? {
            let (p,kind,blob,link,mode,compressed)=r?;
            let content = match blob {
                Some(b) => {
                    let is_c = compressed.unwrap_or(0)!=0;
                    if is_c { decompress_bytes(&b).ok() } else if is_gzipped(&b) { decompress_bytes(&b).ok().or(Some(b)) } else { Some(b) }
                }, None=>None
            };
    ...
    for (db_path, (db_kind,_ ,_,_)) in db_map.iter() {
        ...
        if !host_all_paths.contains(db_path) {
            ...
            if db_kind == "dir" {
                ...
                let _ = conn.execute("DELETE FROM vm_fs WHERE path LIKE ?1", params![format!("{}/%", db_path)]);
            }
        }
    }
    for p in to_delete {
