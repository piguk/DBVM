...
fn main() -> anyhow::Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Closure { path, output } => {
            selfdb_closure(&path, &output)?;
        }
        Cmd::Scan { db, dir } => {
            selfdb_scan(&db, &dir)?;
        }
        Cmd::Userland { output, dirs } => {
            selfdb_userland(&output, dirs)?;
        }
        Cmd::Bundle { path, filter } => {
            bundle_list(&path, &filter)?;
        }
        Cmd::BundleInfo { path } => {
            bundle_info(&path)?;
        }
        Cmd::Pack {
            ...
            if !no_bundle {
                cmd.push("--bundle".to_string());
            }
            if no_sections {
                cmd.push("--no-sections".to_string());
            }
            if no_notes {
                cmd.push("--no-notes".to_string());
            }
            println!("{}", cmd.join(" "));
            let info = selfdb::elf::parse_elf(&input, no_sections, no_notes)?;
            selfdb::db::create_self_db(&output, &info, no_sections, no_notes)?;
        }
        Cmd::Run { path, rest } => {
            let exe = std::env::var("SELF_EXEC")
                .unwrap_or_else(|_| "target/release/self-exec".to_string());
        ...
        Cmd::VmInit { db, force } => {
            let _c = selfdb::vm::init_vm_db(&db, force)?;
            println!("vm init -> {} (WAL+mmap+compress)", db);
        }
        Cmd::VmAdd { db, host, vm_path } => {
            let c = Connection::open(&db)?;
            selfdb::vm::vm_add_file(&c, &host, &vm_path)?;
            println!("add {} -> {}:{}", host, db, vm_path);
        }
        Cmd::VmPack {
            db,
            host_dir,
            ...
            let c = Connection::open(&db)?;
            let n = selfdb::vm::vm_pack_host_dir(&c, &host_dir, &prefix)?;
            println!("pack {} -> {}:{} ({} files)", host_dir, db, prefix, n);
        }
        Cmd::VmImport { db, elf, prefix } => {
            let c = Connection::open(&db)?;
            let n = selfdb::vm::vm_import_closure(&c, &elf, &prefix)?;
            println!("import {} -> {} ({} objects)", elf, db, n);
        }
        Cmd::VmImportRootfs { db, tar, strip } => {
            let c = selfdb::vm::vm_open(&db)?;
            let n = selfdb::vm::vm_import_tar(&c, &tar, &strip)?;
            println!("import-tar {} -> {} ({} entries)", tar, db, n);
        }
        Cmd::VmMaterialize { db, dest } => {
            let c = selfdb::vm::vm_open(&db)?;
            let n = selfdb::vm::vm_materialize_tree(&c, std::path::Path::new(&dest))?;
            println!("materialize {} -> {} ({} files)", db, dest, n);
        }
        Cmd::VmLs { db, path } => {
            let c = selfdb::vm::vm_open(&db)?;
            for (p, k, m, sz, _) in selfdb::vm::vm_ls(&c, &path)? {
                println!("{:8} {:4} {:>10}  {}", k, format!("{:o}", m), sz, p);
            }
        }
        Cmd::VmCat { db, path } => {
            let c = selfdb::vm::vm_open(&db)?;
            let data = selfdb::vm::vm_cat(&c, &path)?;
            use std::io::Write;
            std::io::stdout().write_all(&data)?;
        }
        Cmd::VmStat { db, path } => {
            let c = selfdb::vm::vm_open(&db)?;
            let (p, k, m, sz, mtime, hash) = selfdb::vm::vm_stat(&c, &path)?;
            println!(
                "path={} kind={} mode={:o} size={} mtime={} hash={}",
                p, k, m, sz, mtime, hash
            );
        }
        Cmd::VmExec { db, vm_path, rest } => {
            ...
            let tmp = if p.is_null() {
                std::env::temp_dir().join(format!("vm-exec-{}", std::process::id()))
            } else {
            ...
            let interp: Option<String> =
                selfdb::elf::parse_elf(&host_bin.to_string_lossy(), true, true)
                    .ok()
        ...
        Cmd::VmChroot { db, cmd, rest } => {
            ...
            let tmp = if pp.is_null() {
                std::env::temp_dir().join(format!("vm-chroot-{}", std::process::id()))
            } else {
            ...
            let status = if has_bwrap {
                ...
                match s {
                    ...
                    _ => {
                        ...
                        cmd2.status().unwrap_or_else(|e| {
                            eprintln!("unshare/chroot failed: {}", e);
                            std::process::exit(127)
                        })
                    }
                }
                ...
                cmd.status().unwrap_or_else(|e| {
                    eprintln!("chroot failed (need root): {}", e);
                    std::process::exit(127)
                })
            };
        ...
        Cmd::VmCheckpoint { db, name, note } => {
            let c = selfdb::vm::vm_open(&db)?;
            selfdb::vm::vm_checkpoint(&c, &name, &note)?;
            println!("checkpoint {} @ {}", name, db);
        }
        Cmd::VmSnapshots { db } => {
            let c = Connection::open(&db)?;
            for (id, name, ts, pc, bytes, note) in selfdb::vm::vm_list_snapshots(&c)? {
                println!(
                    "[{}] {} ts={} pc={} bytes={} note={}",
        ...
        Cmd::VmVerify { db } => {
            let c = Connection::open(&db)?;
            println!("{}", selfdb::vm::vm_verify(&c)?);
        }
        Cmd::VmExtract { db, vm_path, out } => {
            let c = selfdb::vm::vm_open(&db)?;
            let data = selfdb::vm::vm_cat(&c, &vm_path)?;
            std::fs::write(&out, &data)?;
            println!("extract {} -> {} ({} bytes)", vm_path, out, data.len());
        }
        Cmd::VmMemInsert {
            db,
            addr,
            ...
            selfdb::vm::vm_mem_insert(&c, a, sz, pr, &data)?;
            println!(
        ...
        Cmd::VmMemList { db } => {
            let c = Connection::open(&db)?;
            for (id, addr, size, prot) in selfdb::vm::vm_mem_list(&c)? {
                println!("[{}] addr=0x{:x} size={} prot={}", id, addr, size, prot);
            }
        }
        Cmd::VmMemClear { db } => {
            let c = Connection::open(&db)?;
            selfdb::vm::vm_mem_clear(&c)?;
            println!("mem clear -> {}", db);
        }
        Cmd::VmSnapshotFile { db, name } => {
            let c = selfdb::vm::vm_open(&db)?;
            let p = selfdb::vm::vm_snapshot_file(&c, &db, &name)?;
            println!("snapshot-file {} -> {}", db, p);
        }
        Cmd::VmRestoreFile { db, name } => {
            selfdb::vm::vm_restore_file(&db, &name)?;
            println!("restore-file {} @ {}", name, db);
        }
        Cmd::VmSync { db, host_root } => {
            let c = selfdb::vm::vm_open(&db)?;
            let (cr, up, del) =
                selfdb::vm::vm_sync_from_host(&c, std::path::Path::new(&host_root))?;
            let _ = selfdb::vm::vm_apply_pragmas(&c);
            println!(
                "sync {} <- {}: +{} ~{} -{} (WAL+NORMAL+mmap)",
                db, host_root, cr, up, del
            );
        }
        Cmd::VmGc { db } => {
            let c = selfdb::vm::vm_open(&db)?;
            let (before, saved) = selfdb::vm::vm_gc(&c)?;
...
fn selfdb_scan(db: &str, dir: &str) -> anyhow::Result<()> {
    ...
    for exe in &exes {
        if let std::collections::hash_map::Entry::Vacant(slot) = seen.entry(exe.clone()) {
            let son = selfdb::elf::soname_for_path_cached(slot.key(), &mut meta_cache);
