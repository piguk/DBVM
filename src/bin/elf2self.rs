...
fn main() -> Result<()> {
    ...
    if args.bundle {
        ...
        extra = format!(
            " bundle_objects={} bundle_needs={}",
            bundle_objects.0, bundle_objects.1
        );
    }
    println!("wrote {}", args.output);
    println!(
        "  segments: {}, symbols: {}, sections: {}, notes: {}, dynamic: {}, needed: {}, relocs: {}{}",
        info.e_phnum,
        info.symbols.len(),
        info.sections.len(),
        info.notes.len(),
        info.dynamic_entries.len(),
        info.needed.len(),
        info.relocs.len(),
        extra
    );
...
fn bundle_closure(input: &str, output: &str, info: &dbvm::elf::ElfInfo) -> Result<(usize, usize)> {
    ...
    let add = |path: PathBuf,
               kind: &str,
               order: &mut Vec<(PathBuf, Option<String>, String)>,
               seen: &mut FxHashMap<PathBuf, String>,
               meta_cache: &mut FxHashMap<PathBuf, ElfMeta>| {
        let rp = std::fs::canonicalize(&path).unwrap_or(path.clone());
        if seen.contains_key(&rp) {
            return;
        }
        let soname = if kind == "lib" {
            soname_of_cached(&rp, meta_cache)
        } else {
            None
        };
    ...
    if let Some(p) = interp {
        ...
        if pb.is_file() {
            add(
                std::fs::canonicalize(&pb).unwrap_or(pb.clone()),
                "lib",
                &mut order,
                &mut seen,
                &mut meta_cache,
    ...
    for (idx, (rp, soname, kind)) in order.iter().enumerate() {
        ...
        let id = (idx + 1) as i64;
        conn.execute(
            "INSERT INTO bundle_objects VALUES (?1,?2,?3,?4,?5,?6,?7)",
            params![
                id,
    ...
    for (rp, _, _) in &order {
        if let Some(oid) = path_to_id.get(rp) {
                ...
                .enumerate()
            {
                ...
                conn.execute(
                    "INSERT INTO bundle_needs VALUES (?1,?2,?3,?4)",
                    params![oid, n as i64, soname, resolved],
                )?;
                needs += 1;
            }
        }
